...
Assessed release candidate: generator version 21 (`2026-07-14`). All evidence below is
...
| Automated regression and release       |        7/8 | A frozen Docker install runs formatting, spelling, lint, catalog checks, TypeScript, 172 unit/integration tests, production build, PWA checks, and budgets. Twenty-seven no-retry Chromium/Firefox/WebKit journeys pass. V8 coverage is 84.89% statements, 78.90% branches, 84.22% functions, and 86.79% lines. Mutation testing remains. |
...
- `docker compose run --rm --build verify`: clean frozen install plus formatting, configured
  spelling, lint, icon catalog, 100-template catalog, TypeScript, 172 tests, production build,
  bundle budgets, manifest, service worker, and precache verification.
- `docker compose run --rm --build e2e`: 27 mobile journeys across Chromium, Firefox, and WebKit,
  with no retries.
- `docker compose exec -T dev pnpm run test:coverage`: all 172 tests pass with the coverage values
  above.
